...
// Категории лекарств
const medicineCategories = ['painkillers', 'antibiotics', 'cold', 'heart', 'digestive', 'antifungal', 'allergy'];

async function loadProducts() {
    const grid = document.getElementById('productsGrid');
    if (!grid) return;
    
    grid.innerHTML = '<div class="loading-spinner"><i class="fas fa-spinner fa-pulse"></i> Загрузка лекарств...</div>';
    ...
    try {
        ...
        // Фильтруем только лекарства (по категориям из списка)
        allProducts = data.filter(p => medicineCategories.includes(p.category));
...
function createProductCard(product) {
    ...
    card.innerHTML = `
        ...
        <div class="product-image">
            <i class="fas ${product.icon || 'fa-tablets'}"></i>
        </div>
