...
async function loadProducts() {
    ...
    try {
        let url = `${API_URL}/medicines/filter?category=children`;
...
function applyFiltersAndSort() {
    ...
    // Фильтр по возрасту
    if (currentAge !== 'all') {
        filtered = filtered.filter(p => p.age === currentAge);
    }
...
function createProductCard(product) {
    ...
    card.innerHTML = `
        ...
        ${product.age ? `<span class="age-badge">${ageLabels[product.age] || product.age}</span>` : ''}
        <div class="product-image">
            <i class="fas ${product.icon || 'fa-baby'}"></i>
        </div>
        ...
        <div class="product-footer">
            <div>
                ...
                <span class="in-stock">${product.in_stock ? '✅ В наличии' : '❌ Нет в наличии'}</span>
            </div>
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Фильтр по возрасту
    document.querySelectorAll('#ageFilterButtons .filter-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            document.querySelectorAll('#ageFilterButtons .filter-btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
