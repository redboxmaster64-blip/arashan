...
async function loadProducts() {
    ...
    grid.innerHTML = '<div class="loading-spinner"><i class="fas fa-spinner fa-pulse"></i> Загрузка медтехники...</div>';
    
    try {
        const resp = await fetch(`${API_URL}/medicines/filter?category=medical`);
...
function applyFiltersAndSort() {
    ...
    if (currentDevice !== 'all') {
        filtered = filtered.filter(p => p.deviceType === currentDevice);
    }
...
function createProductCard(product) {
    ...
    const deviceLabels = {
        ...
        mechanical: '⚙️ Механический',
        accessory: '📎 Аксессуар'
    };
    
    card.innerHTML = `
        ${product.badge ? `<span class="product-badge">${escapeHtml(product.badge)}</span>` : ''}
        ${product.deviceType ? `<span class="device-badge">${deviceLabels[product.deviceType] || product.deviceType}</span>` : ''}
        <div class="product-image">
            <i class="fas ${product.icon || 'fa-shield-alt'}"></i>
        </div>
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Фильтр по категориям (типам товаров)
    ...
    document.querySelectorAll('#deviceFilterButtons .filter-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            document.querySelectorAll('#deviceFilterButtons .filter-btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
